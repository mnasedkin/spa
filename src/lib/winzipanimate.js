/**
 *Для подключения стилей необходимо присвоить id = winzip-animation для всего блока анимации
 */
class Winzipanimate {
    ...
    menedger(_this) {
        ...
        //анимация горизонтального текста в gallery
        let text = document.getElementById('direction');
        text.style.transform = _this.drawCDirection(_this, text);
    ...
    drawBottomAnimation(_this, startPosition, deltaOffset) {
        ...
        if (nextPosition >= _this.animationBlock.offsetHeight - _this.boxFractions[0].height) {
            ...
            _this.top_img.src = 'images/reggie_thumbs_up.png';
        } else {
            _this.winZipBlock.setAttribute('fell', 'false');
            _this.top_img.src = 'images/wz_top.png'
        }
...
//     let startAnimation = new Winzipanimate();
